...
calcula_ocup_desocup <- function(mesano){
  pnadc <- pnadc_design(read_pnadc(paste0("C:/Users/italo/Desktop/trimestral/txt/PNADC_0",mesano,".txt"), 
    ...
    update(
      ...
      #Homem
      horastodoshabit_h = ifelse(pia %in% 1  & V2007==1, rend2, NA),
      horastodosefetiv_h = ifelse(pia %in% 1 & V2007==1, rend3, NA),
      
      #Mulher
      horastodoshabit_m = ifelse(pia %in% 1  & V2007==2, rend2, NA),
      horastodosefetiv_m = ifelse(pia %in% 1 & V2007==2, rend3, NA),
  ...
  ## Total - Habitual Homem
  estimativas_1 <- svyby(~horastodoshabit_h, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_1)[3]<-"cv_1"
  
  ## Total - Habitual Mulher
  estimativas_2 <- svyby(~horastodoshabit_m, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  ...
  ## Total - Todos Efetivo Mulher
  estimativas_3 <- svyby(~horastodosefetiv_h, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_3)[3]<-"cv_3"
  
  ## Total - Todos Efetivo
  estimativas_4 <- svyby(~horastodosefetiv_m, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  ...
  total_1<- svymean(~horastodoshabit_h, subset(pnadc,UF=="31"), na.rm = TRUE)
  total_2<- svymean(~horastodoshabit_m, subset(pnadc,UF=="31"), na.rm = TRUE)
  total_3<- svymean(~horastodosefetiv_h, subset(pnadc,UF=="31"), na.rm = TRUE)
  total_4<- svymean(~horastodosefetiv_m, subset(pnadc,UF=="31"), na.rm = TRUE)
  ...
  total_mg<-tibble(
    ...
    horastodoshabit_h = coef(total_1),
    cv_1 = round(cv(total_1)*100,1),
    
    horastodoshabit_m = coef(total_2),
    ...
    horastodosefetiv_h = coef(total_3),
    cv_3 = round(cv(total_3)*100,1),
    
    horastodosefetiv_m = coef(total_4),
  ...
  rm(estimativas_t1, estimativas_t2, estimativas_1, estimativas_2, estimativas_3, estimativas_4, pnadc)
  gc()
...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39,52)], function(df) {
                                       data.frame(
                                         ...
                                         "Habitualmente em todos os trabalhos - homens" = as.numeric(df[9,4]),
                                         "cv" = as.numeric(df[9,5]),
                                         "Habitualmente em todos os trabalhos - mulheres" = as.numeric(df[9, 6]),
                                         "cv" = as.numeric(df[9,7]),
                                         "Total - efetivamente em todos os trabalhos" = as.numeric(df[9, 8]),
                                         "cv" = as.numeric(df[9,9]),
                                         "Efetivamente em todos os trabalhos - homens" = as.numeric(df[9,10]),
                                         "cv" = as.numeric(df[9,11]),
                                         "Efetivamente em todos os trabalhos - mulheres" = as.numeric(df[9,12]),
...
bh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39,52)], function(df) {
                                       data.frame(
                                         ...
                                         "Habitualmente em todos os trabalhos - homens" = as.numeric(df[1,4]),
                                         "cv" = as.numeric(df[1,5]),
                                         "Habitualmente em todos os trabalhos - mulheres" = as.numeric(df[1, 6]),
                                         "cv" = as.numeric(df[1,7]),
                                         "Total - efetivamente em todos os trabalhos" = as.numeric(df[1, 8]),
                                         "cv" = as.numeric(df[1,9]),
                                         "Efetivamente em todos os trabalhos - homens" = as.numeric(df[1,10]),
                                         "cv" = as.numeric(df[1,11]),
                                         "Efetivamente em todos os trabalhos - mulheres" = as.numeric(df[1,12]),
...
entornobhcolar<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                            ...
                                            13,26,39,52)], function(df) {
                                              data.frame(
                                                ...
                                                "Habitualmente em todos os trabalhos - homens" = as.numeric(df[2,4]),
                                                "cv" = as.numeric(df[2,5]),
                                                "Habitualmente em todos os trabalhos - mulheres" = as.numeric(df[2, 6]),
                                                "cv" = as.numeric(df[2,7]),
                                                "Total - efetivamente em todos os trabalhos" = as.numeric(df[2, 8]),
                                                "cv" = as.numeric(df[2,9]),
                                                "Efetivamente em todos os trabalhos - homens" = as.numeric(df[2,10]),
                                                "cv" = as.numeric(df[2,11]),
                                                "Efetivamente em todos os trabalhos - mulheres" = as.numeric(df[2,12]),
...
sulmg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                        ...
                                        13,26,39,52)], function(df) {
                                          data.frame(
                                            ...
                                            "Habitualmente em todos os trabalhos - homens" = as.numeric(df[3,4]),
                                            "cv" = as.numeric(df[3,5]),
                                            "Habitualmente em todos os trabalhos - mulheres" = as.numeric(df[3, 6]),
                                            "cv" = as.numeric(df[3,7]),
                                            "Total - efetivamente em todos os trabalhos" = as.numeric(df[3, 8]),
                                            "cv" = as.numeric(df[3,9]),
                                            "Efetivamente em todos os trabalhos - homens" = as.numeric(df[3,10]),
                                            "cv" = as.numeric(df[3,11]),
                                            "Efetivamente em todos os trabalhos - mulheres" = as.numeric(df[3,12]),
...
trng<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39,52)], function(df) {
                                         data.frame(
                                           ...
                                           "Habitualmente em todos os trabalhos - homens" = as.numeric(df[4,4]),
                                           "cv" = as.numeric(df[4,5]),
                                           "Habitualmente em todos os trabalhos - mulheres" = as.numeric(df[4, 6]),
                                           "cv" = as.numeric(df[4,7]),
                                           "Total - efetivamente em todos os trabalhos" = as.numeric(df[4, 8]),
                                           "cv" = as.numeric(df[4,9]),
                                           "Efetivamente em todos os trabalhos - homens" = as.numeric(df[4,10]),
                                           "cv" = as.numeric(df[4,11]),
                                           "Efetivamente em todos os trabalhos - mulheres" = as.numeric(df[4,12]),
...
zonamata<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                           ...
                                           13,26,39,52)], function(df) {
                                             data.frame(
                                               ...
                                               "Habitualmente em todos os trabalhos - homens" = as.numeric(df[5,4]),
                                               "cv" = as.numeric(df[5,5]),
                                               "Habitualmente em todos os trabalhos - mulheres" = as.numeric(df[5, 6]),
                                               "cv" = as.numeric(df[5,7]),
                                               "Total - efetivamente em todos os trabalhos" = as.numeric(df[5, 8]),
                                               "cv" = as.numeric(df[5,9]),
                                               "Efetivamente em todos os trabalhos - homens" = as.numeric(df[5,10]),
                                               "cv" = as.numeric(df[5,11]),
                                               "Efetivamente em todos os trabalhos - mulheres" = as.numeric(df[5,12]),
...
nortemgride<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39,52)], function(df) {
                                            data.frame(
                                              ...
                                              "Habitualmente em todos os trabalhos - homens" = as.numeric(df[6,4]),
                                              "cv" = as.numeric(df[6,5]),
                                              "Habitualmente em todos os trabalhos - mulheres" = as.numeric(df[6, 6]),
                                              "cv" = as.numeric(df[6,7]),
                                              "Total - efetivamente em todos os trabalhos" = as.numeric(df[6, 8]),
                                              "cv" = as.numeric(df[6,9]),
                                              "Efetivamente em todos os trabalhos - homens" = as.numeric(df[6,10]),
                                              "cv" = as.numeric(df[6,11]),
                                              "Efetivamente em todos os trabalhos - mulheres" = as.numeric(df[6,12]),
...
riodoce<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39,52)], function(df) {
                                            data.frame(
                                              ...
                                              "Habitualmente em todos os trabalhos - homens" = as.numeric(df[7,4]),
                                              "cv" = as.numeric(df[7,5]),
                                              "Habitualmente em todos os trabalhos - mulheres" = as.numeric(df[7, 6]),
                                              "cv" = as.numeric(df[7,7]),
                                              "Total - efetivamente em todos os trabalhos" = as.numeric(df[7, 8]),
                                              "cv" = as.numeric(df[7,9]),
                                              "Efetivamente em todos os trabalhos - homens" = as.numeric(df[7,10]),
                                              "cv" = as.numeric(df[7,11]),
                                              "Efetivamente em todos os trabalhos - mulheres" = as.numeric(df[7,12]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39,52)], function(df) {
                                            data.frame(
                                              ...
                                              "Habitualmente em todos os trabalhos - homens" = as.numeric(df[8,4]),
                                              "cv" = as.numeric(df[8,5]),
                                              "Habitualmente em todos os trabalhos - mulheres" = as.numeric(df[8, 6]),
                                              "cv" = as.numeric(df[8,7]),
                                              "Total - efetivamente em todos os trabalhos" = as.numeric(df[8, 8]),
                                              "cv" = as.numeric(df[8,9]),
                                              "Efetivamente em todos os trabalhos - homens" = as.numeric(df[8,10]),
                                              "cv" = as.numeric(df[8,11]),
                                              "Efetivamente em todos os trabalhos - mulheres" = as.numeric(df[8,12]),
...
baseestratatualV6371<-list("01-Belo Horizonte"=bh,"02-Entorno metropolitano + Colar de BH"=entornobhcolar,
...
saveRDS(baseestratatualV6371,file = "C:/Users/italo/Desktop/trimestral/baseestratatualV6371.rds")
