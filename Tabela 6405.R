...
calcula_ocup_desocup <- function(mesano){
  ...
  pnadc <- pnadc_design(data_pnadc=pnadc)  %>%
    ...
    update(
      ...
      #Habitual Principal
      VD4016a = ifelse(pia==1 & VD4002==1 & V2010==1, VD4016rph, NA),
      VD4016b = ifelse(pia==1 & VD4002==1 & (V2010==2 | V2010==4), VD4016rph, NA),
      
      #Efetivo principal
      VD4017a = ifelse(pia==1 & VD4002==1 & V2010==1, VD4017rpe, NA),
      VD4017b = ifelse(pia==1 & VD4002==1 & (V2010==2 | V2010==4), VD4017rpe, NA),
      
      #Habitual Todos
      VD4019a = ifelse(pia==1 & VD4002==1 & V2010==1, VD4019rth, NA),
      VD4019b = ifelse(pia==1 & VD4002==1 & (V2010==2 | V2010==4), VD4019rth, NA),

      #Efetivo todos
      VD4020a = ifelse(pia==1 & VD4002==1 & V2010==1, VD4020rte, NA),
      VD4020b = ifelse(pia==1 & VD4002==1 & (V2010==2 | V2010==4), VD4020rte, NA),
  ...
  estimativas_3 <- svyby(~VD4017a, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_3)[3]<-"cv_3"
  
  estimativas_4 <- svyby(~VD4017b, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  ...
  estimativas_5 <- svyby(~VD4019a, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_5)[3]<-"cv_5"
  
  estimativas_6 <- svyby(~VD4019b, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  ...
  estimativas_7 <- svyby(~VD4020a, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_7)[3]<-"cv_7"
  
  estimativas_8 <- svyby(~VD4020b, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  ...
  total_ocupada3<- svymean(~VD4017a, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada4<- svymean(~VD4017b, subset(pnadc, UF=="31"),na.rm = TRUE)
  
  total_ocupada5<- svymean(~VD4019a, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada6<- svymean(~VD4019b, subset(pnadc, UF=="31"),na.rm = TRUE)
  
  total_ocupada7<- svymean(~VD4020a, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada8<- svymean(~VD4020b, subset(pnadc, UF=="31"),na.rm = TRUE)
...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39,52)], function(df) {
                                       data.frame(
                                         ...
                                         "Rend. médio mensal habitual - Trab principal - Brancos" = as.numeric(df[9,4]),
                                         "cv" = as.numeric(df[9,5]),
                                         "Rend. médio mensal habitual - Trab principal - Negros" = as.numeric(df[9,6]),
                                         "cv" = as.numeric(df[9,7]),
                                         
                                         
                                         "Rend. médio mensal efetivo - Trab principal - Total" = as.numeric(df[9,8]),
                                         "cv" = as.numeric(df[9,9]),
                                         "Rend. médio mensal efetivo - Trab principal - Brancos" = as.numeric(df[9,10]),
                                         "cv" = as.numeric(df[9,11]),
                                         "Rend. médio mensal efetivo - Trab principal - Negros" = as.numeric(df[9,12]),
                                         ...
                                         "Rend. médio mensal habitual - Todos os trabalhos - Brancos" = as.numeric(df[9,16]),
                                         "cv" = as.numeric(df[9,17]),
                                         "Rend. médio mensal habitual - Todos os trabalhos - Negros" = as.numeric(df[9,18]),
                                         "cv" = as.numeric(df[9,19]),
                                         
                                         
                                         "Rend. médio mensal efetivo - Todos os trabalhos - Total" = as.numeric(df[9,20]),
                                         "cv" = as.numeric(df[9,21]),
                                         "Rend. médio mensal efetivo - Todos os trabalhos - Brancos" = as.numeric(df[9,22]),
                                         "cv" = as.numeric(df[9,23]),
                                         "Rend. médio mensal efetivo - Todos os trabalhos - Negros" = as.numeric(df[9,24]),
...
bh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39,52)], function(df) {
                                       data.frame(
                                         ...
                                         "Rend. médio mensal habitual - Trab principal - Brancos" = as.numeric(df[1,4]),
                                         "cv" = as.numeric(df[1,5]),
                                         "Rend. médio mensal habitual - Trab principal - Negros" = as.numeric(df[1,6]),
                                         "cv" = as.numeric(df[1,7]),
                                         
                                         
                                         "Rend. médio mensal efetivo - Trab principal - Total" = as.numeric(df[1,8]),
                                         "cv" = as.numeric(df[1,9]),
                                         "Rend. médio mensal efetivo - Trab principal - Brancos" = as.numeric(df[1,10]),
                                         "cv" = as.numeric(df[1,11]),
                                         "Rend. médio mensal efetivo - Trab principal - Negros" = as.numeric(df[1,12]),
                                         ...
                                         "Rend. médio mensal habitual - Todos os trabalhos - Brancos" = as.numeric(df[1,16]),
                                         "cv" = as.numeric(df[1,17]),
                                         "Rend. médio mensal habitual - Todos os trabalhos - Negros" = as.numeric(df[1,18]),
                                         "cv" = as.numeric(df[1,19]),
                                         
                                         
                                         "Rend. médio mensal efetivo - Todos os trabalhos - Total" = as.numeric(df[1,20]),
                                         "cv" = as.numeric(df[1,21]),
                                         "Rend. médio mensal efetivo - Todos os trabalhos - Brancos" = as.numeric(df[1,22]),
                                         "cv" = as.numeric(df[1,23]),
                                         "Rend. médio mensal efetivo - Todos os trabalhos - Negros" = as.numeric(df[1,24]),
...
entornobhcolar<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                            ...
                                            13,26,39,52)], function(df) {
                                              data.frame(
                                                ...
                                                "Rend. médio mensal habitual - Trab principal - Brancos" = as.numeric(df[2,4]),
                                                "cv" = as.numeric(df[2,5]),
                                                "Rend. médio mensal habitual - Trab principal - Negros" = as.numeric(df[2,6]),
                                                "cv" = as.numeric(df[2,7]),
                                                
                                                
                                                "Rend. médio mensal efetivo - Trab principal - Total" = as.numeric(df[2,8]),
                                                "cv" = as.numeric(df[2,9]),
                                                "Rend. médio mensal efetivo - Trab principal - Brancos" = as.numeric(df[2,10]),
                                                "cv" = as.numeric(df[2,11]),
                                                "Rend. médio mensal efetivo - Trab principal - Negros" = as.numeric(df[2,12]),
                                                ...
                                                "Rend. médio mensal habitual - Todos os trabalhos - Brancos" = as.numeric(df[2,16]),
                                                "cv" = as.numeric(df[2,17]),
                                                "Rend. médio mensal habitual - Todos os trabalhos - Negros" = as.numeric(df[2,18]),
                                                "cv" = as.numeric(df[2,19]),
                                                
                                                
                                                "Rend. médio mensal efetivo - Todos os trabalhos - Total" = as.numeric(df[2,20]),
                                                "cv" = as.numeric(df[2,21]),
                                                "Rend. médio mensal efetivo - Todos os trabalhos - Brancos" = as.numeric(df[2,22]),
                                                "cv" = as.numeric(df[2,23]),
                                                "Rend. médio mensal efetivo - Todos os trabalhos - Negros" = as.numeric(df[2,24]),
...
sulmg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                        ...
                                        13,26,39,52)], function(df) {
                                          data.frame(
                                            ...
                                            "Rend. médio mensal habitual - Trab principal - Brancos" = as.numeric(df[3,4]),
                                            "cv" = as.numeric(df[3,5]),
                                            "Rend. médio mensal habitual - Trab principal - Negros" = as.numeric(df[3,6]),
                                            "cv" = as.numeric(df[3,7]),
                                            
                                            
                                            "Rend. médio mensal efetivo - Trab principal - Total" = as.numeric(df[3,8]),
                                            "cv" = as.numeric(df[3,9]),
                                            "Rend. médio mensal efetivo - Trab principal - Brancos" = as.numeric(df[3,10]),
                                            "cv" = as.numeric(df[3,11]),
                                            "Rend. médio mensal efetivo - Trab principal - Negros" = as.numeric(df[3,12]),
                                            ...
                                            "Rend. médio mensal habitual - Todos os trabalhos - Brancos" = as.numeric(df[3,16]),
                                            "cv" = as.numeric(df[3,17]),
                                            "Rend. médio mensal habitual - Todos os trabalhos - Negros" = as.numeric(df[3,18]),
                                            "cv" = as.numeric(df[3,19]),
                                            
                                            
                                            "Rend. médio mensal efetivo - Todos os trabalhos - Total" = as.numeric(df[3,20]),
                                            "cv" = as.numeric(df[3,21]),
                                            "Rend. médio mensal efetivo - Todos os trabalhos - Brancos" = as.numeric(df[3,22]),
                                            "cv" = as.numeric(df[3,23]),
                                            "Rend. médio mensal efetivo - Todos os trabalhos - Negros" = as.numeric(df[3,24]),
...
trng<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39,52)], function(df) {
                                         data.frame(
                                           ...
                                           "Rend. médio mensal habitual - Trab principal - Brancos" = as.numeric(df[4,4]),
                                           "cv" = as.numeric(df[4,5]),
                                           "Rend. médio mensal habitual - Trab principal - Negros" = as.numeric(df[4,6]),
                                           "cv" = as.numeric(df[4,7]),
                                           
                                           
                                           "Rend. médio mensal efetivo - Trab principal - Total" = as.numeric(df[4,8]),
                                           "cv" = as.numeric(df[4,9]),
                                           "Rend. médio mensal efetivo - Trab principal - Brancos" = as.numeric(df[4,10]),
                                           "cv" = as.numeric(df[4,11]),
                                           "Rend. médio mensal efetivo - Trab principal - Negros" = as.numeric(df[4,12]),
                                           ...
                                           "Rend. médio mensal habitual - Todos os trabalhos - Brancos" = as.numeric(df[4,16]),
                                           "cv" = as.numeric(df[4,17]),
                                           "Rend. médio mensal habitual - Todos os trabalhos - Negros" = as.numeric(df[4,18]),
                                           "cv" = as.numeric(df[4,19]),
                                           
                                           
                                           "Rend. médio mensal efetivo - Todos os trabalhos - Total" = as.numeric(df[4,20]),
                                           "cv" = as.numeric(df[4,21]),
                                           "Rend. médio mensal efetivo - Todos os trabalhos - Brancos" = as.numeric(df[4,22]),
                                           "cv" = as.numeric(df[4,23]),
                                           "Rend. médio mensal efetivo - Todos os trabalhos - Negros" = as.numeric(df[4,24]),
...
zonamata<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                           ...
                                           13,26,39,52)], function(df) {
                                             data.frame(
                                               ...
                                               "Rend. médio mensal habitual - Trab principal - Brancos" = as.numeric(df[5,4]),
                                               "cv" = as.numeric(df[5,5]),
                                               "Rend. médio mensal habitual - Trab principal - Negros" = as.numeric(df[5,6]),
                                               "cv" = as.numeric(df[5,7]),
                                               
                                               
                                               "Rend. médio mensal efetivo - Trab principal - Total" = as.numeric(df[5,8]),
                                               "cv" = as.numeric(df[5,9]),
                                               "Rend. médio mensal efetivo - Trab principal - Brancos" = as.numeric(df[5,10]),
                                               "cv" = as.numeric(df[5,11]),
                                               "Rend. médio mensal efetivo - Trab principal - Negros" = as.numeric(df[5,12]),
                                               ...
                                               "Rend. médio mensal habitual - Todos os trabalhos - Brancos" = as.numeric(df[5,16]),
                                               "cv" = as.numeric(df[5,17]),
                                               "Rend. médio mensal habitual - Todos os trabalhos - Negros" = as.numeric(df[5,18]),
                                               "cv" = as.numeric(df[5,19]),
                                               
                                               
                                               "Rend. médio mensal efetivo - Todos os trabalhos - Total" = as.numeric(df[5,20]),
                                               "cv" = as.numeric(df[5,21]),
                                               "Rend. médio mensal efetivo - Todos os trabalhos - Brancos" = as.numeric(df[5,22]),
                                               "cv" = as.numeric(df[5,23]),
                                               "Rend. médio mensal efetivo - Todos os trabalhos - Negros" = as.numeric(df[5,24]),
...
nortemgride<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39,52)], function(df) {
                                            data.frame(
                                              ...
                                              "Rend. médio mensal habitual - Trab principal - Brancos" = as.numeric(df[6,4]),
                                              "cv" = as.numeric(df[6,5]),
                                              "Rend. médio mensal habitual - Trab principal - Negros" = as.numeric(df[6,6]),
                                              "cv" = as.numeric(df[6,7]),
                                              
                                              
                                              "Rend. médio mensal efetivo - Trab principal - Total" = as.numeric(df[6,8]),
                                              "cv" = as.numeric(df[6,9]),
                                              "Rend. médio mensal efetivo - Trab principal - Brancos" = as.numeric(df[6,10]),
                                              "cv" = as.numeric(df[6,11]),
                                              "Rend. médio mensal efetivo - Trab principal - Negros" = as.numeric(df[6,12]),
                                              ...
                                              "Rend. médio mensal habitual - Todos os trabalhos - Brancos" = as.numeric(df[6,16]),
                                              "cv" = as.numeric(df[6,17]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Negros" = as.numeric(df[6,18]),
                                              "cv" = as.numeric(df[6,19]),
                                              
                                              
                                              "Rend. médio mensal efetivo - Todos os trabalhos - Total" = as.numeric(df[6,20]),
                                              "cv" = as.numeric(df[6,21]),
                                              "Rend. médio mensal efetivo - Todos os trabalhos - Brancos" = as.numeric(df[6,22]),
                                              "cv" = as.numeric(df[6,23]),
                                              "Rend. médio mensal efetivo - Todos os trabalhos - Negros" = as.numeric(df[6,24]),
...
riodoce<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39,52)], function(df) {
                                            data.frame(
                                              ...
                                              "Rend. médio mensal habitual - Trab principal - Brancos" = as.numeric(df[7,4]),
                                              "cv" = as.numeric(df[7,5]),
                                              "Rend. médio mensal habitual - Trab principal - Negros" = as.numeric(df[7,6]),
                                              "cv" = as.numeric(df[7,7]),
                                              
                                              
                                              "Rend. médio mensal efetivo - Trab principal - Total" = as.numeric(df[7,8]),
                                              "cv" = as.numeric(df[7,9]),
                                              "Rend. médio mensal efetivo - Trab principal - Brancos" = as.numeric(df[7,10]),
                                              "cv" = as.numeric(df[7,11]),
                                              "Rend. médio mensal efetivo - Trab principal - Negros" = as.numeric(df[7,12]),
                                              ...
                                              "Rend. médio mensal habitual - Todos os trabalhos - Brancos" = as.numeric(df[7,16]),
                                              "cv" = as.numeric(df[7,17]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Negros" = as.numeric(df[7,18]),
                                              "cv" = as.numeric(df[7,19]),
                                              
                                              
                                              "Rend. médio mensal efetivo - Todos os trabalhos - Total" = as.numeric(df[7,20]),
                                              "cv" = as.numeric(df[7,21]),
                                              "Rend. médio mensal efetivo - Todos os trabalhos - Brancos" = as.numeric(df[7,22]),
                                              "cv" = as.numeric(df[7,23]),
                                              "Rend. médio mensal efetivo - Todos os trabalhos - Negros" = as.numeric(df[7,24]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39,52)], function(df) {
                                            data.frame(
                                              ...
                                              "Rend. médio mensal habitual - Trab principal - Brancos" = as.numeric(df[8,4]),
                                              "cv" = as.numeric(df[8,5]),
                                              "Rend. médio mensal habitual - Trab principal - Negros" = as.numeric(df[8,6]),
                                              "cv" = as.numeric(df[8,7]),
                                              
                                              
                                              "Rend. médio mensal efetivo - Trab principal - Total" = as.numeric(df[8,8]),
                                              "cv" = as.numeric(df[8,9]),
                                              "Rend. médio mensal efetivo - Trab principal - Brancos" = as.numeric(df[8,10]),
                                              "cv" = as.numeric(df[8,11]),
                                              "Rend. médio mensal efetivo - Trab principal - Negros" = as.numeric(df[8,12]),
                                              ...
                                              "Rend. médio mensal habitual - Todos os trabalhos - Brancos" = as.numeric(df[8,16]),
                                              "cv" = as.numeric(df[8,17]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Negros" = as.numeric(df[8,18]),
                                              "cv" = as.numeric(df[8,19]),
                                              
                                              
                                              "Rend. médio mensal efetivo - Todos os trabalhos - Total" = as.numeric(df[8,20]),
                                              "cv" = as.numeric(df[8,21]),
                                              "Rend. médio mensal efetivo - Todos os trabalhos - Brancos" = as.numeric(df[8,22]),
                                              "cv" = as.numeric(df[8,23]),
                                              "Rend. médio mensal efetivo - Todos os trabalhos - Negros" = as.numeric(df[8,24]),
...
baseestratatualrendm6405<-list("01-Belo Horizonte"=bh,"02-Entorno metropolitano + Colar de BH"=entornobhcolar,
...
saveRDS(baseestratatualrendm6405,file = "C:/Users/italo/Desktop/trimestral/baseestratatualrendm6405.rds")
