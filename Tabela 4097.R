...
#Tabela 4097 - Pessoas de 14 anos ou mais de idade, ocupadas na semana de referência, por posição na ocupação e categoria do emprego no trabalho principa
...
calcula_ocup_desocup <- function(mesano){
  pnadc <- pnadc_design(read_pnadc(paste0("C:/Users/italo/Desktop/trimestral/txt/PNADC_0",mesano,".txt"), 
    ...
    update(
      #Criação das variáveis para população, conforme variável VD4009
      pia = as.numeric(V2009 >= 14),
      
      ocupada_ct = 1 * (pia == 1 & (VD4009=="01" | VD4009=="03")),
      ocupada_st = 1 * (pia == 1 & (VD4009=="02" | VD4009=="04")),
      ocupada_sp = 1 * (pia == 1 & (VD4009=="05" | VD4009=="06" | VD4009=="07")),
      ocupada_op = 1 * (pia == 1 & (VD4009=="08" | VD4009=="09" | VD4009=="10")),

      one = 1 * (ocupada_ct==1 | ocupada_st==1 | ocupada_sp==1 | ocupada_op==1),
  ...
  estimativas_1 <- svyby(~ocupada_ct, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_1)[3]<-"cv_1"
  
  ## Total - Trabalhador doméstico
  estimativas_2 <- svyby(~ocupada_st, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_2)[3]<-"cv_2"
  
  ## Total - Empregado no setor público
  estimativas_3 <- svyby(~ocupada_sp, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_3)[3]<-"cv_3"
  
  ## Total - Outros
  estimativas_4 <- svyby(~ocupada_op, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  ...
  total_1<- svytotal(~ocupada_ct, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_2<- svytotal(~ocupada_st, subset(pnadc,UF=="31"), na.rm = TRUE)
  total_3<- svytotal(~ocupada_sp, subset(pnadc,UF=="31"), na.rm = TRUE)
  total_4<- svytotal(~ocupada_op, subset(pnadc,UF=="31"), na.rm = TRUE)
  ...
  total_mg<-tibble(
    ...
    ocupada_ct = coef(total_1),
    cv_1 = round(cv(total_1)*100,1),

    ocupada_st = coef(total_2),
    cv_2 = round(cv(total_2)*100,1),
    
    ocupada_sp = coef(total_3),
    cv_3 = round(cv(total_3)*100,1),
    
    ocupada_op = coef(total_4),
...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39,52)], function(df) {
                                       data.frame(
                                         ...
                                         "Empregado e trabalhador familiar com carteira assinada" = as.numeric(df[9,4]),
                                         "cv" = as.numeric(df[9,5]),
                                         "Empregado e trabalhador familiar sem carteira assinada" = as.numeric(df[9,6]),
                                         "cv" = as.numeric(df[9,7]),
                                         "Empregado no setor público" = as.numeric(df[9,8]),
                                         "cv" = as.numeric(df[9,9]),
                                         "Outros" = as.numeric(df[9,10]),
...
bh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39,52)], function(df) {
                                       data.frame(
                                         ...
                                         "Empregado e trabalhador familiar com carteira assinada" = as.numeric(df[1,4]),
                                         "cv" = as.numeric(df[1,5]),
                                         "Empregado e trabalhador familiar sem carteira assinada" = as.numeric(df[1,6]),
                                         "cv" = as.numeric(df[1,7]),
                                         "Empregado no setor público" = as.numeric(df[1,8]),
                                         "cv" = as.numeric(df[1,9]),
                                         "Outros" = as.numeric(df[1,10]),
...
entornobhcolar<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                            ...
                                            13,26,39,52)], function(df) {
                                              data.frame(
                                                ...
                                                "Empregado e trabalhador familiar com carteira assinada" = as.numeric(df[2,4]),
                                                "cv" = as.numeric(df[2,5]),
                                                "Empregado e trabalhador familiar sem carteira assinada" = as.numeric(df[2,6]),
                                                "cv" = as.numeric(df[2,7]),
                                                "Empregado no setor público" = as.numeric(df[2,8]),
                                                "cv" = as.numeric(df[2,9]),
                                                "Outros" = as.numeric(df[2,10]),
...
sulmg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                        ...
                                        13,26,39,52)], function(df) {
                                          data.frame(
                                            ...
                                            "Empregado e trabalhador familiar com carteira assinada" = as.numeric(df[3,4]),
                                            "cv" = as.numeric(df[3,5]),
                                            "Empregado e trabalhador familiar sem carteira assinada" = as.numeric(df[3,6]),
                                            "cv" = as.numeric(df[3,7]),
                                            "Empregado no setor público" = as.numeric(df[3,8]),
                                            "cv" = as.numeric(df[3,9]),
                                            "Outros" = as.numeric(df[3,10]),
...
trng<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39,52)], function(df) {
                                         data.frame(
                                           ...
                                           "Empregado e trabalhador familiar com carteira assinada" = as.numeric(df[4,4]),
                                           "cv" = as.numeric(df[4,5]),
                                           "Empregado e trabalhador familiar sem carteira assinada" = as.numeric(df[4,6]),
                                           "cv" = as.numeric(df[4,7]),
                                           "Empregado no setor público" = as.numeric(df[4,8]),
                                           "cv" = as.numeric(df[4,9]),
                                           "Outros" = as.numeric(df[4,10]),
...
zonamata<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                           ...
                                           13,26,39,52)], function(df) {
                                             data.frame(
                                               ...
                                               "Empregado e trabalhador familiar com carteira assinada" = as.numeric(df[5,4]),
                                               "cv" = as.numeric(df[5,5]),
                                               "Empregado e trabalhador familiar sem carteira assinada" = as.numeric(df[5,6]),
                                               "cv" = as.numeric(df[5,7]),
                                               "Empregado no setor público" = as.numeric(df[5,8]),
                                               "cv" = as.numeric(df[5,9]),
                                               "Outros" = as.numeric(df[5,10]),
...
nortemgride<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39,52)], function(df) {
                                            data.frame(
                                              ...
                                              "Empregado e trabalhador familiar com carteira assinada" = as.numeric(df[6,4]),
                                              "cv" = as.numeric(df[6,5]),
                                              "Empregado e trabalhador familiar sem carteira assinada" = as.numeric(df[6,6]),
                                              "cv" = as.numeric(df[6,7]),
                                              "Empregado no setor público" = as.numeric(df[6,8]),
                                              "cv" = as.numeric(df[6,9]),
                                              "Outros" = as.numeric(df[6,10]),
...
riodoce<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39,52)], function(df) {
                                            data.frame(
                                              ...
                                              "Empregado e trabalhador familiar com carteira assinada" = as.numeric(df[7,4]),
                                              "cv" = as.numeric(df[7,5]),
                                              "Empregado e trabalhador familiar sem carteira assinada" = as.numeric(df[7,6]),
                                              "cv" = as.numeric(df[7,7]),
                                              "Empregado no setor público" = as.numeric(df[7,8]),
                                              "cv" = as.numeric(df[7,9]),
                                              "Outros" = as.numeric(df[7,10]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39,52)], function(df) {
                                            data.frame(
                                              ...
                                              "Empregado e trabalhador familiar com carteira assinada" = as.numeric(df[8,4]),
                                              "cv" = as.numeric(df[8,5]),
                                              "Empregado e trabalhador familiar sem carteira assinada" = as.numeric(df[8,6]),
                                              "cv" = as.numeric(df[8,7]),
                                              "Empregado no setor público" = as.numeric(df[8,8]),
                                              "cv" = as.numeric(df[8,9]),
                                              "Outros" = as.numeric(df[8,10]),
...
baseestratatualV4097<-list("01-Belo Horizonte"=bh,"02-Entorno metropolitano + Colar de BH"=entornobhcolar,
