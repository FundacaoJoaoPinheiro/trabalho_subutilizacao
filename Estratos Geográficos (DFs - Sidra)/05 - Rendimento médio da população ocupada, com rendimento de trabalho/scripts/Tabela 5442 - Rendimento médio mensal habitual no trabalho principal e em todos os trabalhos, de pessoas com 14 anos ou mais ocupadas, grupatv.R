...
calcula_ocup_desocup <- function(ano){
  ...
  estimativas_8 <- svyby(~renda_trabalho_princ_8, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_8)[3]<-"cv_8"
  
  estimativas_9 <- svyby(~renda_trabalho_princ_9, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_9)[3]<-"cv_9"
  
  estimativas_10 <- svyby(~renda_trabalho_princ_10, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_10)[3]<-"cv_10"
  
  estimativas_11 <- svyby(~renda_trabalho_princ_11, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_11)[3]<-"cv_11"
  
  estimativas_12 <- svyby(~renda_trabalho_princ_12, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
