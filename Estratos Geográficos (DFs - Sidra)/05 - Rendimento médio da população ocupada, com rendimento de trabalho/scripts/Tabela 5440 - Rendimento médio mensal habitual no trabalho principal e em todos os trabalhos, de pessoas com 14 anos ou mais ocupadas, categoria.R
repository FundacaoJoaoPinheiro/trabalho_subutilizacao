...
calcula_ocup_desocup <- function(ano){
  pnadc <- pnadc_design(read_pnadc(paste0("C:/Users/italo/Desktop/MICRODADOS/txt/PNADC_",ano,".txt"), 
                                   paste0("C:/Users/italo/Desktop/MICRODADOS/doc/input_PNADC_",ano,".txt"),vars=c("VD4002","V2007", "V2009", "V2010", "VD4019", "VD4015", "VD4007", "VD4016", "VD3004", "VD4009"))) %>% 
    update(
           #Criando as variáveis de renda
      
           pia = as.numeric(V2009 >= 14),
           ocupados = ifelse(pia == 1, as.numeric(VD4002 %in% 1) , NA) ,
           categ = as.factor(VD4009),
        
           renda_trabalho_princ_1 = ifelse(ocupados %in% 1 & categ == 1, VD4016, NA),
           renda_trabalho_princ_2 = ifelse(ocupados %in% 1 & categ == 2, VD4016, NA),
           renda_trabalho_princ_3 = ifelse(ocupados %in% 1 & categ == 3, VD4016, NA),
           renda_trabalho_princ_4 = ifelse(ocupados %in% 1 & categ == 4, VD4016, NA),
           renda_trabalho_princ_5 = ifelse(ocupados %in% 1 & categ == 5, VD4016, NA),
           renda_trabalho_princ_6 = ifelse(ocupados %in% 1 & categ == 6, VD4016, NA),
           renda_trabalho_princ_7 = ifelse(ocupados %in% 1 & categ == 7, VD4016, NA),
           renda_trabalho_princ_8 = ifelse(ocupados %in% 1 & categ == 8, VD4016, NA),
           renda_trabalho_princ_9 = ifelse(ocupados %in% 1 & categ == 9, VD4016, NA),
  ...
  rm(estimativas_1, estimativas_2, estimativas_3, estimativas_4, estimativas_5, estimativas_6, estimativas_7, estimativas_8, estimativas_9, pnadc)
  gc()
