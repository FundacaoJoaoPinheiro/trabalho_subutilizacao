...
calcula_ocup_desocup <- function(ano){
  ...
  colnames(estimativas_dm)[3]<-"cv_dm"
  ...
  colnames(estimativas_tdh)[3]<-"cv_td_homem"
  ...
  colnames(estimativas_tdm)[3]<-"cv_td_mulher"
  ...
  total_mg<-tibble(
    ...
    #se_oh = SE(total_ocupadah),
    ocupadam = coef(total_ocupadam),
    #se_om = SE(total_ocupadam),
    desocupadah = coef(total_desocupadah),
    #se_dh = SE(total_desocupadah),
    desocupadam = coef(total_desocupadam),
    #se_dm = SE(total_desocupadam),
    tx_desocupadah = coef(total_tx_desocupacaoh),
    #se_tdh = SE(total_tx_desocupacaoh),
    tx_desocupadam = coef(total_tx_desocupacaom),
    #se_tdm = SE(total_tx_desocupacaom),
    periodo = paste0(ano)
  )
...
saveRDS(baseestratatualsex,file = "C:/Users/italo/Desktop/MICRODADOS/baseestratatualsex.rds")
