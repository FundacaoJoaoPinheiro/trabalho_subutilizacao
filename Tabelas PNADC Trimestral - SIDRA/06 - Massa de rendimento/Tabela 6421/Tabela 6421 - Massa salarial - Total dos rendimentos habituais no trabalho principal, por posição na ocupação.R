...
#Tabela 6421 - Massa salarial, rendimentos habituais, por posição na ocupação do trabalho principal
...
calcula_ocup_desocup <- function(mesano){
  pnadc <- pnadc_design(read_pnadc(paste0("C:/Users/italo/Desktop/trimestral/txt/PNADC_0",mesano,".txt"), 
                                   "C:/Users/italo/Desktop/trimestral/input/input_PNADC_trimestral.txt",vars=c("VD4002", "V2009", "VD4001", "V2007", "V2010", "VD3004", "V4076", "VD4015", "VD4019", "VD4007"))) %>%
    ...
    update(
           ...
           VD4019a = ifelse(ocupados %in% 1 & VD4007 == 1, VD4019, NA),
           VD4019b = ifelse(ocupados %in% 1 & VD4007 == 2, VD4019, NA),
           VD4019c = ifelse(ocupados %in% 1 & VD4007 == 3, VD4019, NA),
  ...
  estimativas_1 <- svyby(~VD4019a, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_1)[3]<-"cv_1"
  
  estimativas_2 <- svyby(~VD4019b, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_2)[3]<-"cv_2"
  
  estimativas_3 <- svyby(~VD4019c, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  ...
  total_ocupada1<- svytotal(~VD4019a, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada2<- svytotal(~VD4019b, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada3<- svytotal(~VD4019c, subset(pnadc, UF=="31"),na.rm = TRUE)
  ...
  total_mg<-tibble(
    ...
    VD4019a = coef(total_ocupada1),
    cv_1 = round(cv(total_ocupada1)*100,1),
    VD4019b = coef(total_ocupada2),
    cv_2 = round(cv(total_ocupada2)*100,1),
    VD4019c = coef(total_ocupada3),
...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Massa salarial -  Empregado (R$) " = as.numeric(df[11,2]),
                                         "cv_1" = as.numeric(df[11,3]),
                                         "Massa salarial -  Empregador (R$) " = as.numeric(df[11,4]),
                                         "cv_2" = as.numeric(df[11,5]),
                                         "Massa salarial -  Conta própria (R$) " = as.numeric(df[11,6]),
...
bh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Massa salarial -  Empregado (R$) " = as.numeric(df[1,2]),
                                         "cv_1" = as.numeric(df[1,3]),
                                         "Massa salarial -  Empregador (R$) " = as.numeric(df[1,4]),
                                         "cv_2" = as.numeric(df[1,5]),
                                         "Massa salarial -  Conta própria (R$) " = as.numeric(df[1,6]),
...
entornobh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                            ...
                                            13,26,39)], function(df) {
                                              data.frame(
                                                ...
                                                "Massa salarial -  Empregado (R$) " = as.numeric(df[2,2]),
                                                "cv_1" = as.numeric(df[2,3]),
                                                "Massa salarial -  Empregador (R$) " = as.numeric(df[2,4]),
                                                "cv_2" = as.numeric(df[2,5]),
                                                "Massa salarial -  Conta própria (R$) " = as.numeric(df[2,6]),
...
colarbh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Massa salarial -  Empregado (R$) " = as.numeric(df[3,2]),
                                              "cv_1" = as.numeric(df[3,3]),
                                              "Massa salarial -  Empregador (R$) " = as.numeric(df[3,4]),
                                              "cv_2" = as.numeric(df[3,5]),
                                              "Massa salarial -  Conta própria (R$) " = as.numeric(df[3,6]),
...
RIDE<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Massa salarial -  Empregado (R$) " = as.numeric(df[4,2]),
                                           "cv_1" = as.numeric(df[4,3]),
                                           "Massa salarial -  Empregador (R$) " = as.numeric(df[4,4]),
                                           "cv_2" = as.numeric(df[4,5]),
                                           "Massa salarial -  Conta própria (R$) " = as.numeric(df[4,6]),
...
sulmg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                        ...
                                        13,26,39)], function(df) {
                                          data.frame(
                                            ...
                                            "Massa salarial -  Empregado (R$) " = as.numeric(df[5,2]),
                                            "cv_1" = as.numeric(df[5,3]),
                                            "Massa salarial -  Empregador (R$) " = as.numeric(df[5,4]),
                                            "cv_2" = as.numeric(df[5,5]),
                                            "Massa salarial -  Conta própria (R$) " = as.numeric(df[5,6]),
...
trng<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Massa salarial -  Empregado (R$) " = as.numeric(df[6,2]),
                                           "cv_1" = as.numeric(df[6,3]),
                                           "Massa salarial -  Empregador (R$) " = as.numeric(df[6,4]),
                                           "cv_2" = as.numeric(df[6,5]),
                                           "Massa salarial -  Conta própria (R$) " = as.numeric(df[6,6]),
...
zonamata<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                           ...
                                           13,26,39)], function(df) {
                                             data.frame(
                                               ...
                                               "Massa salarial -  Empregado (R$) " = as.numeric(df[7,2]),
                                               "cv_1" = as.numeric(df[7,3]),
                                               "Massa salarial -  Empregador (R$) " = as.numeric(df[7,4]),
                                               "cv_2" = as.numeric(df[7,5]),
                                               "Massa salarial -  Conta própria (R$) " = as.numeric(df[7,6]),
...
nortemg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Massa salarial -  Empregado (R$) " = as.numeric(df[8,2]),
                                              "cv_1" = as.numeric(df[8,3]),
                                              "Massa salarial -  Empregador (R$) " = as.numeric(df[8,4]),
                                              "cv_2" = as.numeric(df[8,5]),
                                              "Massa salarial -  Conta própria (R$) " = as.numeric(df[8,6]),
...
riodoce<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Massa salarial -  Empregado (R$) " = as.numeric(df[9,2]),
                                              "cv_1" = as.numeric(df[9,3]),
                                              "Massa salarial -  Empregador (R$) " = as.numeric(df[9,4]),
                                              "cv_2" = as.numeric(df[9,5]),
                                              "Massa salarial -  Conta própria (R$) " = as.numeric(df[9,6]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Massa salarial -  Empregado (R$) " = as.numeric(df[10,2]),
                                              "cv_1" = as.numeric(df[10,3]),
                                              "Massa salarial -  Empregador (R$) " = as.numeric(df[10,4]),
                                              "cv_2" = as.numeric(df[10,5]),
                                              "Massa salarial -  Conta própria (R$) " = as.numeric(df[10,6]),
...
baseestratatualmassapos<-list("01-Belo Horizonte"=bh,"02-Entorno metropolitano de BH"=entornobh,"03-Colar metropolitano de BH"=colarbh,
...
saveRDS(baseestratatualmassapos,file = "C:/Users/italo/Desktop/trimestral/baseestratatualmassapos.rds")
