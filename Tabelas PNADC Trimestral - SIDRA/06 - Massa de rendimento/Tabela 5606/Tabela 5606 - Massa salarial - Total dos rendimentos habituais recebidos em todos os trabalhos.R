...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39,52)], function(df) {
                                       data.frame(
                                         "Período" = as.character(df[11, 6]),
                                         "Massa salarial - Habitual (Milhões de R$) " = as.numeric(df[11,2]),
                                         "cv_1" = as.numeric(df[11,3]),
                                         "Massa salarial - Efetiva (Milhões de R$) " = as.numeric(df[11,4]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39,52)], function(df) {
                                            data.frame(
                                              "Período" = as.character(df[10, 6]),
                                              "Massa salarial - Habitual (Milhões de R$) " = as.numeric(df[10,2]),
                                              "cv_1" = as.numeric(df[10,3]),
                                              "Massa salarial - Efetiva (Milhões de R$) " = as.numeric(df[10,4]),
...
baseestratatualmassa5606<-list("01-Belo Horizonte"=bh,"02-Entorno metropolitano de BH"=entornobh,"03-Colar metropolitano de BH"=colarbh,
