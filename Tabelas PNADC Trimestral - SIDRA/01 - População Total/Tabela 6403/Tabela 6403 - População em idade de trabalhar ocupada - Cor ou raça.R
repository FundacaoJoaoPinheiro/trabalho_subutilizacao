...
#Tabela 6403 - População ocupada, por cor ou raça
...
calcula_ocup_desocup <- function(mesano){
  pnadc <- pnadc_design(read_pnadc(paste0("C:/Users/italo/Desktop/trimestral/txt/PNADC_0",mesano,".txt"), 
    ...
    update(
           #Criando variáveis por cor ou raça, conforme variável V2010
           ocupadap = 1 * (VD4002 == 1 & (V2010 == 2 | V2010 == 4)),
           ocupaday = 1 * (VD4002 == 1 & (V2010 == 3)),
           ocupadai = 1 * (VD4002 == 1 & (V2010 == 5 | V2010 == 9)),
           ocupadab = 1 * (VD4002 == 1 & (V2010 == 1)),
  ...
  ## Total - Preta ou Parda
  estimativas_1 <- svyby(~ocupadap, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_1)[3]<-"cv_1"
  
  ## Total - Amarela
  estimativas_2 <- svyby(~ocupaday, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_2)[3]<-"cv_2"
  
  ## Total - Indigena
  estimativas_3 <- svyby(~ocupadai, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_3)[3]<-"cv_3"
  
  ## Total - Branca
  estimativas_4 <- svyby(~ocupadab, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  ...
  total_ocupada1<- svytotal(~ocupadap, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada2<- svytotal(~ocupaday, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada3<- svytotal(~ocupadai, subset(pnadc,UF=="31"), na.rm = TRUE)
  total_ocupada4<- svytotal(~ocupadab, subset(pnadc,UF=="31"), na.rm = TRUE)
  ...
  total_mg<-tibble(
    ...
    ocupadap = coef(total_ocupada1),
    cv_1 = round(cv(total_ocupada1)*100,1),
    ocupaday = coef(total_ocupada2),
    cv_2 = round(cv(total_ocupada2)*100,1),
    ocupadai = coef(total_ocupada3),
    cv_3 = round(cv(total_ocupada3)*100,1),
    ocupadab = coef(total_ocupada4),
...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Total - Preta ou Parda" = as.numeric(df[11,2]),
                                         "cv_1" = as.numeric(df[11,3]),
                                         "Total - Amarela" = as.numeric(df[11, 4]),
                                         "cv_2" = as.numeric(df[11,5]),
                                         "Total - Indígena" = as.numeric(df[11, 6]),
                                         "cv_3" = as.numeric(df[11,7]),
                                         "Total - Branca" = as.numeric(df[11,8]),
...
bh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Total - Preta ou Parda" = as.numeric(df[1,2]),
                                         "cv_1" = as.numeric(df[1,3]),
                                         "Total - Amarela" = as.numeric(df[1, 4]),
                                         "cv_2" = as.numeric(df[1,5]),
                                         "Total - Indígena" = as.numeric(df[1, 6]),
                                         "cv_3" = as.numeric(df[1,7]),
                                         "Total - Branca" = as.numeric(df[1,8]),
...
entornobh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                            ...
                                            13,26,39)], function(df) {
                                              data.frame(
                                                ...
                                                "Total - Preta ou Parda" = as.numeric(df[2,2]),
                                                "cv_1" = as.numeric(df[2,3]),
                                                "Total - Amarela" = as.numeric(df[2, 4]),
                                                "cv_2" = as.numeric(df[2,5]),
                                                "Total - Indígena" = as.numeric(df[2, 6]),
                                                "cv_3" = as.numeric(df[2,7]),
                                                "Total - Branca" = as.numeric(df[2,8]),
...
colarbh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Preta ou Parda" = as.numeric(df[3,2]),
                                              "cv_1" = as.numeric(df[3,3]),
                                              "Total - Amarela" = as.numeric(df[3, 4]),
                                              "cv_2" = as.numeric(df[3,5]),
                                              "Total - Indígena" = as.numeric(df[3, 6]),
                                              "cv_3" = as.numeric(df[3,7]),
                                              "Total - Branca" = as.numeric(df[3,8]),
...
RIDE<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Total - Preta ou Parda" = as.numeric(df[4,2]),
                                           "cv_1" = as.numeric(df[4,3]),
                                           "Total - Amarela" = as.numeric(df[4, 4]),
                                           "cv_2" = as.numeric(df[4,5]),
                                           "Total - Indígena" = as.numeric(df[4, 6]),
                                           "cv_3" = as.numeric(df[4,7]),
                                           "Total - Branca" = as.numeric(df[4,8]),
...
sulmg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                        ...
                                        13,26,39)], function(df) {
                                          data.frame(
                                            ...
                                            "Total - Preta ou Parda" = as.numeric(df[5,2]),
                                            "cv_1" = as.numeric(df[5,3]),
                                            "Total - Amarela" = as.numeric(df[5, 4]),
                                            "cv_2" = as.numeric(df[5,5]),
                                            "Total - Indígena" = as.numeric(df[5, 6]),
                                            "cv_3" = as.numeric(df[5,7]),
                                            "Total - Branca" = as.numeric(df[5,8]),
...
trng<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Total - Preta ou Parda" = as.numeric(df[6,2]),
                                           "cv_1" = as.numeric(df[6,3]),
                                           "Total - Amarela" = as.numeric(df[6, 4]),
                                           "cv_2" = as.numeric(df[6,5]),
                                           "Total - Indígena" = as.numeric(df[6, 6]),
                                           "cv_3" = as.numeric(df[6,7]),
                                           "Total - Branca" = as.numeric(df[6,8]),
...
zonamata<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                           ...
                                           13,26,39)], function(df) {
                                             data.frame(
                                               ...
                                               "Total - Preta ou Parda" = as.numeric(df[7,2]),
                                               "cv_1" = as.numeric(df[7,3]),
                                               "Total - Amarela" = as.numeric(df[7, 4]),
                                               "cv_2" = as.numeric(df[7,5]),
                                               "Total - Indígena" = as.numeric(df[7, 6]),
                                               "cv_3" = as.numeric(df[7,7]),
                                               "Total - Branca" = as.numeric(df[7,8]),
...
nortemg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Preta ou Parda" = as.numeric(df[8,2]),
                                              "cv_1" = as.numeric(df[8,3]),
                                              "Total - Amarela" = as.numeric(df[8, 4]),
                                              "cv_2" = as.numeric(df[8,5]),
                                              "Total - Indígena" = as.numeric(df[8, 6]),
                                              "cv_3" = as.numeric(df[8,7]),
                                              "Total - Branca" = as.numeric(df[8,8]),
...
riodoce<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Preta ou Parda" = as.numeric(df[9,2]),
                                              "cv_1" = as.numeric(df[9,3]),
                                              "Total - Amarela" = as.numeric(df[9, 4]),
                                              "cv_2" = as.numeric(df[9,5]),
                                              "Total - Indígena" = as.numeric(df[9, 6]),
                                              "cv_3" = as.numeric(df[9,7]),
                                              "Total - Branca" = as.numeric(df[9,8]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Preta ou Parda" = as.numeric(df[10,2]),
                                              "cv_1" = as.numeric(df[10,3]),
                                              "Total - Amarela" = as.numeric(df[10, 4]),
                                              "cv_2" = as.numeric(df[10,5]),
                                              "Total - Indígena" = as.numeric(df[10, 6]),
                                              "cv_3" = as.numeric(df[10,7]),
                                              "Total - Branca" = as.numeric(df[10,8]),
...
baseestratatualcor<-list("01-Belo Horizonte"=bh,"02-Entorno metropolitano de BH"=entornobh,"03-Colar metropolitano de BH"=colarbh,
...
saveRDS(baseestratatualcor,file = "C:/Users/italo/Desktop/trimestral/baseestratatualcor.rds")
