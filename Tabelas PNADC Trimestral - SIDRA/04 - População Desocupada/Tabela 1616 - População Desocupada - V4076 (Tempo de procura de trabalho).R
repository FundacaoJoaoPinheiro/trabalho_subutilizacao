...
calcula_ocup_desocup <- function(mesano){
  pnadc <- pnadc_design(read_pnadc(paste0("C:/Users/italo/Desktop/trimestral/txt/PNADC_0",mesano,".txt"), 
                                   "C:/Users/italo/Desktop/trimestral/input/input_PNADC_trimestral.txt",vars=c("VD4002", "V2009", "VD4001", "V2007", "V2010", "VD3004", "V4076"))) %>%
    
    update(
           #Criação das variáveis para população desocupada, conforme variável V4076
           desocupada1 = 1 * (VD4002 == 2 & V4076 == 1),
           desocupada2 = 1 * (VD4002 == 2 & V4076 == 2),
           desocupada3 = 1 * (VD4002 == 2 & V4076 == 3),
           desocupada4 = 1 * (VD4002 == 2 & V4076 == 4),
  ...
  ## Total - Menos de 1 mês
  estimativas_1 <- svyby(~desocupada1, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_1)[3]<-"cv_1"
  
  ## Total - De 1 mês a menos de 1 ano
  estimativas_2 <- svyby(~desocupada2, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_2)[3]<-"cv_2"
  
  ## Total - Indigena
  estimativas_3 <- svyby(~desocupada3, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_3)[3]<-"cv_3"
  
  ## Total - 2 anos ou mais
  estimativas_4 <- svyby(~desocupada4, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  ...
  total_desocupada1<- svytotal(~desocupada1, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_desocupada2<- svytotal(~desocupada2, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_desocupada3<- svytotal(~desocupada3, subset(pnadc,UF=="31"), na.rm = TRUE)
  total_desocupada4<- svytotal(~desocupada4, subset(pnadc,UF=="31"), na.rm = TRUE)
  ...
  total_mg<-tibble(
    ...
    desocupada1 = coef(total_desocupada1),
    cv_1 = round(cv(total_desocupada1)*100,1),
    desocupada2 = coef(total_desocupada2),
    cv_2 = round(cv(total_desocupada2)*100,1),
    desocupada3 = coef(total_desocupada3),
    cv_3 = round(cv(total_desocupada3)*100,1),
    desocupada4 = coef(total_desocupada4),
    cv_4 = round(cv(total_desocupada4)*100,1),
...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Total - Menos de 1 mês" = as.numeric(df[11,2]),
                                         "cv_1" = as.numeric(df[11,3]),
                                         "Total - De 1 mês a menos de 1 ano" = as.numeric(df[11, 4]),
                                         "cv_2" = as.numeric(df[11,5]),
                                         "Total - De 1 ano a menos de 2 anos" = as.numeric(df[11, 6]),
                                         "cv_3" = as.numeric(df[11,7]),
                                         "Total - 2 anos ou mais" = as.numeric(df[11,8]),
...
bh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Total - Menos de 1 mês" = as.numeric(df[1,2]),
                                         "cv_1" = as.numeric(df[1,3]),
                                         "Total - De 1 mês a menos de 1 ano" = as.numeric(df[1, 4]),
                                         "cv_2" = as.numeric(df[1,5]),
                                         "Total - De 1 ano a menos de 2 anos" = as.numeric(df[1, 6]),
                                         "cv_3" = as.numeric(df[1,7]),
                                         "Total - 2 anos ou mais" = as.numeric(df[1,8]),
...
entornobh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                            ...
                                            13,26,39)], function(df) {
                                              data.frame(
                                                ...
                                                "Total - Menos de 1 mês" = as.numeric(df[2,2]),
                                                "cv_1" = as.numeric(df[2,3]),
                                                "Total - De 1 mês a menos de 1 ano" = as.numeric(df[2, 4]),
                                                "cv_2" = as.numeric(df[2,5]),
                                                "Total - De 1 ano a menos de 2 anos" = as.numeric(df[2, 6]),
                                                "cv_3" = as.numeric(df[2,7]),
                                                "Total - 2 anos ou mais" = as.numeric(df[2,8]),
...
colarbh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Menos de 1 mês" = as.numeric(df[3,2]),
                                              "cv_1" = as.numeric(df[3,3]),
                                              "Total - De 1 mês a menos de 1 ano" = as.numeric(df[3, 4]),
                                              "cv_2" = as.numeric(df[3,5]),
                                              "Total - De 1 ano a menos de 2 anos" = as.numeric(df[3, 6]),
                                              "cv_3" = as.numeric(df[3,7]),
                                              "Total - 2 anos ou mais" = as.numeric(df[3,8]),
...
RIDE<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Total - Menos de 1 mês" = as.numeric(df[4,2]),
                                           "cv_1" = as.numeric(df[4,3]),
                                           "Total - De 1 mês a menos de 1 ano" = as.numeric(df[4, 4]),
                                           "cv_2" = as.numeric(df[4,5]),
                                           "Total - De 1 ano a menos de 2 anos" = as.numeric(df[4, 6]),
                                           "cv_3" = as.numeric(df[4,7]),
                                           "Total - 2 anos ou mais" = as.numeric(df[4,8]),
...
sulmg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                        ...
                                        13,26,39)], function(df) {
                                          data.frame(
                                            ...
                                            "Total - Menos de 1 mês" = as.numeric(df[5,2]),
                                            "cv_1" = as.numeric(df[5,3]),
                                            "Total - De 1 mês a menos de 1 ano" = as.numeric(df[5, 4]),
                                            "cv_2" = as.numeric(df[5,5]),
                                            "Total - De 1 ano a menos de 2 anos" = as.numeric(df[5, 6]),
                                            "cv_3" = as.numeric(df[5,7]),
                                            "Total - 2 anos ou mais" = as.numeric(df[5,8]),
...
trng<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Total - Menos de 1 mês" = as.numeric(df[6,2]),
                                           "cv_1" = as.numeric(df[6,3]),
                                           "Total - De 1 mês a menos de 1 ano" = as.numeric(df[6, 4]),
                                           "cv_2" = as.numeric(df[6,5]),
                                           "Total - De 1 ano a menos de 2 anos" = as.numeric(df[6, 6]),
                                           "cv_3" = as.numeric(df[6,7]),
                                           "Total - 2 anos ou mais" = as.numeric(df[6,8]),
...
zonamata<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                           ...
                                           13,26,39)], function(df) {
                                             data.frame(
                                               ...
                                               "Total - Menos de 1 mês" = as.numeric(df[7,2]),
                                               "cv_1" = as.numeric(df[7,3]),
                                               "Total - De 1 mês a menos de 1 ano" = as.numeric(df[7, 4]),
                                               "cv_2" = as.numeric(df[7,5]),
                                               "Total - De 1 ano a menos de 2 anos" = as.numeric(df[7, 6]),
                                               "cv_3" = as.numeric(df[7,7]),
                                               "Total - 2 anos ou mais" = as.numeric(df[7,8]),
...
nortemg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Menos de 1 mês" = as.numeric(df[8,2]),
                                              "cv_1" = as.numeric(df[8,3]),
                                              "Total - De 1 mês a menos de 1 ano" = as.numeric(df[8, 4]),
                                              "cv_2" = as.numeric(df[8,5]),
                                              "Total - De 1 ano a menos de 2 anos" = as.numeric(df[8, 6]),
                                              "cv_3" = as.numeric(df[8,7]),
                                              "Total - 2 anos ou mais" = as.numeric(df[8,8]),
...
riodoce<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Menos de 1 mês" = as.numeric(df[9,2]),
                                              "cv_1" = as.numeric(df[9,3]),
                                              "Total - De 1 mês a menos de 1 ano" = as.numeric(df[9, 4]),
                                              "cv_2" = as.numeric(df[9,5]),
                                              "Total - De 1 ano a menos de 2 anos" = as.numeric(df[9, 6]),
                                              "cv_3" = as.numeric(df[9,7]),
                                              "Total - 2 anos ou mais" = as.numeric(df[9,8]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Menos de 1 mês" = as.numeric(df[10,2]),
                                              "cv_1" = as.numeric(df[10,3]),
                                              "Total - De 1 mês a menos de 1 ano" = as.numeric(df[10, 4]),
                                              "cv_2" = as.numeric(df[10,5]),
                                              "Total - De 1 ano a menos de 2 anos" = as.numeric(df[10, 6]),
                                              "cv_3" = as.numeric(df[10,7]),
                                              "Total - 2 anos ou mais" = as.numeric(df[10,8]),
...
baseestratatualV4076<-list("01-Belo Horizonte"=bh,"02-Entorno metropolitano de BH"=entornobh,"03-Colar metropolitano de BH"=colarbh,
...
saveRDS(baseestratatualV4076,file = "C:/Users/italo/Desktop/trimestral/baseestratatualV4076.rds")
