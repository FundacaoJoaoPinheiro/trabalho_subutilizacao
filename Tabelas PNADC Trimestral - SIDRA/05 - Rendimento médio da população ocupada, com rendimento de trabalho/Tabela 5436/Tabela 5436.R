...
calcula_ocup_desocup <- function(mesano){
  pnadc <- pnadc_design(read_pnadc(paste0("C:/Users/italo/Desktop/trimestral/txt/PNADC_0",mesano,".txt"), 
                                   "C:/Users/italo/Desktop/trimestral/input/input_PNADC_trimestral.txt",vars=c("VD4002", "V2009", "VD4001", "V2007", "V2010", "VD3004", "V4076", "VD4016", "VD4019", "VD4007"))) %>%
    update(
           ...
           renda_trabalho_princ_homem = ifelse(ocupados %in% 1 & V2007 == 1, VD4016, NA),
           renda_trabalho_princ_mulher = ifelse(ocupados %in% 1 & V2007 == 2, VD4016, NA),
           renda_todos_trab_homem = ifelse(ocupados %in% 1 & V2007 == 1, VD4019, NA),
           renda_todos_trab_mulher = ifelse(ocupados %in% 1 & V2007 == 2, VD4019, NA),
  ...
  estimativas_1 <- svyby(~renda_trabalho_princ_homem, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_1)[3]<-"cv_1"
  
  estimativas_2 <- svyby(~renda_trabalho_princ_mulher, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_2)[3]<-"cv_2"
  
  estimativas_3 <- svyby(~renda_todos_trab_homem, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  colnames(estimativas_3)[3]<-"cv_3"

  estimativas_4 <- svyby(~renda_todos_trab_mulher, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svymean)
  ...
  total_ocupada1<- svymean(~renda_trabalho_princ_homem, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada2<- svymean(~renda_trabalho_princ_mulher, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada3<- svymean(~renda_todos_trab_homem, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada4<- svymean(~renda_todos_trab_mulher, subset(pnadc, UF=="31"),na.rm = TRUE)
  ...
  total_mg<-tibble(
    ...
    renda_trabalho_princ_homem = coef(total_ocupada1),
    cv_1 = round(cv(total_ocupada1)*100,1),
    renda_trabalho_princ_mulher = coef(total_ocupada2),
    cv_2 = round(cv(total_ocupada2)*100,1),
    renda_todos_trab_homem = coef(total_ocupada3),
    cv_3 = round(cv(total_ocupada3)*100,1),
    renda_todos_trab_mulher = coef(total_ocupada4),
...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Rend. médio mensal habitual - Trab principal - Homem" = as.numeric(df[11,2]),
                                         "cv_1" = as.numeric(df[11,3]),
                                         "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[11,4]),
                                         "cv_2" = as.numeric(df[11,5]),
                                         "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[11,6]),
                                         "cv_3" = as.numeric(df[11,7]),
                                         "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[11,8]),
...
bh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Rend. médio mensal habitual - Trab principal - Homem " = as.numeric(df[1,2]),
                                         "cv_1" = as.numeric(df[1,3]),
                                         "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[1,4]),
                                         "cv_2" = as.numeric(df[1,5]),
                                         "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[1,6]),
                                         "cv_3" = as.numeric(df[1,7]),
                                         "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[1,8]),
...
entornobh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                            ...
                                            13,26,39)], function(df) {
                                              data.frame(
                                                ...
                                                "Rend. médio mensal habitual - Trab principal - Homem " = as.numeric(df[2,2]),
                                                "cv_1" = as.numeric(df[2,3]),
                                                "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[2,4]),
                                                "cv_2" = as.numeric(df[2,5]),
                                                "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[2,6]),
                                                "cv_3" = as.numeric(df[2,7]),
                                                "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[2,8]),
...
colarbh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Rend. médio mensal habitual - Trab principal - Homem " = as.numeric(df[3,2]),
                                              "cv_1" = as.numeric(df[3,3]),
                                              "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[3,4]),
                                              "cv_2" = as.numeric(df[3,5]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[3,6]),
                                              "cv_3" = as.numeric(df[3,7]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[3,8]),
...
RIDE<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Rend. médio mensal habitual - Trab principal - Homem " = as.numeric(df[4,2]),
                                           "cv_1" = as.numeric(df[4,3]),
                                           "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[4,4]),
                                           "cv_2" = as.numeric(df[4,5]),
                                           "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[4,6]),
                                           "cv_3" = as.numeric(df[4,7]),
                                           "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[4,8]),
...
sulmg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                        ...
                                        13,26,39)], function(df) {
                                          data.frame(
                                            ...
                                            "Rend. médio mensal habitual - Trab principal - Homem " = as.numeric(df[5,2]),
                                            "cv_1" = as.numeric(df[5,3]),
                                            "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[5,4]),
                                            "cv_2" = as.numeric(df[5,5]),
                                            "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[5,6]),
                                            "cv_3" = as.numeric(df[5,7]),
                                            "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[5,8]),
...
trng<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Rend. médio mensal habitual - Trab principal - Homem " = as.numeric(df[6,2]),
                                           "cv_1" = as.numeric(df[6,3]),
                                           "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[6,4]),
                                           "cv_2" = as.numeric(df[6,5]),
                                           "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[6,6]),
                                           "cv_3" = as.numeric(df[6,7]),
                                           "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[6,8]),
...
zonamata<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                           ...
                                           13,26,39)], function(df) {
                                             data.frame(
                                               ...
                                               "Rend. médio mensal habitual - Trab principal - Homem " = as.numeric(df[7,2]),
                                               "cv_1" = as.numeric(df[7,3]),
                                               "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[7,4]),
                                               "cv_2" = as.numeric(df[7,5]),
                                               "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[7,6]),
                                               "cv_3" = as.numeric(df[7,7]),
                                               "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[7,8]),
...
nortemg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Rend. médio mensal habitual - Trab principal - Homem " = as.numeric(df[8,2]),
                                              "cv_1" = as.numeric(df[8,3]),
                                              "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[8,4]),
                                              "cv_2" = as.numeric(df[8,5]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[8,6]),
                                              "cv_3" = as.numeric(df[8,7]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[8,8]),
...
riodoce<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Rend. médio mensal habitual - Trab principal - Homem " = as.numeric(df[9,2]),
                                              "cv_1" = as.numeric(df[9,3]),
                                              "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[9,4]),
                                              "cv_2" = as.numeric(df[9,5]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[9,6]),
                                              "cv_3" = as.numeric(df[9,7]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[9,8]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Rend. médio mensal habitual - Trab principal - Homem " = as.numeric(df[10,2]),
                                              "cv_1" = as.numeric(df[10,3]),
                                              "Rend. médio mensal habitual - Trab principal - Mulher" = as.numeric(df[10,4]),
                                              "cv_2" = as.numeric(df[10,5]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Homem" = as.numeric(df[10,6]),
                                              "cv_3" = as.numeric(df[10,7]),
                                              "Rend. médio mensal habitual - Todos os trabalhos - Mulher" = as.numeric(df[10,8]),
...
baseestratatualrendm5436<-list("01-Belo Horizonte"=bh,"02-Entorno metropolitano de BH"=entornobh,"03-Colar metropolitano de BH"=colarbh,
