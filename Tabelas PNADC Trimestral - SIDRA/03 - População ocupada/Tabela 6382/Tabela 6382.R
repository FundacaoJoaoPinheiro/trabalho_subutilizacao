...
#Tabela 6382 - Pessoas de 14 anos ou mais de idade, ocupadas na semana de referência como militares ou empregados do setor público no trabalho principal, por área do emprego
...
calcula_ocup_desocup <- function(mesano){
  pnadc <- pnadc_design(read_pnadc(paste0("C:/Users/italo/Desktop/trimestral/txt/PNADC_0",mesano,".txt"), 
                                   "C:/Users/italo/Desktop/trimestral/input/input_PNADC_trimestral.txt",vars=c("VD4002", "V4012", "V4014", "VD4010", "VD4032", "VD4031", "VD4035", "VD4011", "VD4007", "VD4009", "VD4001", "V2009", "VD4001", "V2007", "V2010", "VD3004", "V4076"))) %>%
    
    update(
      #Criação das variáveis para população, conforme variável VD4007
      pia = as.numeric(V2009 >= 14),
      
      ocupada1 = 1 * (VD4002==1 & (V4012==2 | V4012==4) & V4014==1),
      ocupada2 = 1 * (VD4002==1 & (V4012==2 | V4012==4) & V4014==2),
      ocupada3 = 1 * (VD4002==1 & (V4012==2 | V4012==4) & V4014==3),
      
      one = 1 * (ocupada1==1 | ocupada2==1 |ocupada3==1),
...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Federal" = as.numeric(df[11,4]),
                                         "cv" = as.numeric(df[11,5]),
                                         "Estadual" = as.numeric(df[11, 6]),
                                         "cv" = as.numeric(df[11,7]),
                                         "Municipal" = as.numeric(df[11, 8]),
...
bh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Federal" = as.numeric(df[1,4]),
                                         "cv" = as.numeric(df[1,5]),
                                         "Estadual" = as.numeric(df[1, 6]),
                                         "cv" = as.numeric(df[1,7]),
                                         "Municipal" = as.numeric(df[1, 8]),
...
entornobh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                            ...
                                            13,26,39)], function(df) {
                                              data.frame(
                                                ...
                                                "Federal" = as.numeric(df[2,4]),
                                                "cv" = as.numeric(df[2,5]),
                                                "Estadual" = as.numeric(df[2, 6]),
                                                "cv" = as.numeric(df[2,7]),
                                                "Municipal" = as.numeric(df[2, 8]),
...
colarbh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Federal" = as.numeric(df[3,4]),
                                              "cv" = as.numeric(df[3,5]),
                                              "Estadual" = as.numeric(df[3, 6]),
                                              "cv" = as.numeric(df[3,7]),
                                              "Municipal" = as.numeric(df[3, 8]),
...
RIDE<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Federal" = as.numeric(df[4,4]),
                                           "cv" = as.numeric(df[4,5]),
                                           "Estadual" = as.numeric(df[4, 6]),
                                           "cv" = as.numeric(df[4,7]),
                                           "Municipal" = as.numeric(df[4, 8]),
...
sulmg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                        ...
                                        13,26,39)], function(df) {
                                          data.frame(
                                            ...
                                            "Federal" = as.numeric(df[5,4]),
                                            "cv" = as.numeric(df[5,5]),
                                            "Estadual" = as.numeric(df[5, 6]),
                                            "cv" = as.numeric(df[5,7]),
                                            "Municipal" = as.numeric(df[5, 8]),
...
trng<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Federal" = as.numeric(df[6,4]),
                                           "cv" = as.numeric(df[6,5]),
                                           "Estadual" = as.numeric(df[6, 6]),
                                           "cv" = as.numeric(df[6,7]),
                                           "Municipal" = as.numeric(df[6, 8]),
...
zonamata<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                           ...
                                           13,26,39)], function(df) {
                                             data.frame(
                                               ...
                                               "Federal" = as.numeric(df[7,4]),
                                               "cv" = as.numeric(df[7,5]),
                                               "Estadual" = as.numeric(df[7, 6]),
                                               "cv" = as.numeric(df[7,7]),
                                               "Municipal" = as.numeric(df[7, 8]),
...
nortemg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Federal" = as.numeric(df[8,4]),
                                              "cv" = as.numeric(df[8,5]),
                                              "Estadual" = as.numeric(df[8, 6]),
                                              "cv" = as.numeric(df[8,7]),
                                              "Municipal" = as.numeric(df[8, 8]),
...
riodoce<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Federal" = as.numeric(df[9,4]),
                                              "cv" = as.numeric(df[9,5]),
                                              "Estadual" = as.numeric(df[9, 6]),
                                              "cv" = as.numeric(df[9,7]),
                                              "Municipal" = as.numeric(df[9, 8]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Federal" = as.numeric(df[10,4]),
                                              "cv" = as.numeric(df[10,5]),
                                              "Estadual" = as.numeric(df[10, 6]),
                                              "cv" = as.numeric(df[10,7]),
                                              "Municipal" = as.numeric(df[10, 8]),
...
baseestratatualV6382<-list("01-Belo Horizonte"=bh,"02-Entorno metropolitano de BH"=entornobh,"03-Colar metropolitano de BH"=colarbh,
...
saveRDS(baseestratatualV6382,file = "C:/Users/italo/Desktop/trimestral/baseestratatualV6382.rds")
