...
calcula_ocup_desocup <- function(mesano){
  pnadc <- pnadc_design(read_pnadc(paste0("C:/Users/italo/Desktop/trimestral/txt/PNADC_0",mesano,".txt"), 
                                   "C:/Users/italo/Desktop/trimestral/input/input_PNADC_trimestral.txt",vars=c("VD4002", "V4040", "V2009", "VD4001", "V2007", "V2010", "VD3004", "V4076"))) %>%
    
    update(
           #Criação das variáveis para população desocupada, conforme variável V4040
           
           ocupada1 = 1 * (VD4002 == 1 & V4040 == 1),
           ocupada2 = 1 * (VD4002 == 1 & V4040 == 2),
           ocupada3 = 1 * (VD4002 == 1 & V4040 == 3),
           ocupada4 = 1 * (VD4002 == 1 & V4040 == 4),
           
           one = 1 * (ocupada1==1 | ocupada2==1 | ocupada3==1 | ocupada4==1),
  ...
  ## Total - De 1 ano a menos de 2 anos
...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         "Período" = as.character(df[11, 12]),
                                         "Total" = as.numeric(df[11,2]),
                                         "cv" = as.numeric(df[11,3]),
                                         "Total - Menos de 1 mês" = as.numeric(df[11,4]),
                                         "cv_1" = as.numeric(df[11,5]),
                                         "Total - De 1 mês a menos de 1 ano" = as.numeric(df[11, 6]),
                                         "cv_2" = as.numeric(df[11,7]),
                                         "Total - De 1 ano a menos de 2 anos" = as.numeric(df[11, 8]),
                                         "cv_3" = as.numeric(df[11,9]),
                                         "Total - 2 anos ou mais" = as.numeric(df[11,10]),
...
bh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Total - Menos de 1 mês" = as.numeric(df[1,4]),
                                         "cv_1" = as.numeric(df[1,5]),
                                         "Total - De 1 mês a menos de 1 ano" = as.numeric(df[1, 6]),
                                         "cv_2" = as.numeric(df[1,7]),
                                         "Total - De 1 ano a menos de 2 anos" = as.numeric(df[1, 8]),
                                         "cv_3" = as.numeric(df[1,9]),
                                         "Total - 2 anos ou mais" = as.numeric(df[1,10]),
...
entornobh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                            ...
                                            13,26,39)], function(df) {
                                              data.frame(
                                                ...
                                                "Total - Menos de 1 mês" = as.numeric(df[2,4]),
                                                "cv_1" = as.numeric(df[2,5]),
                                                "Total - De 1 mês a menos de 1 ano" = as.numeric(df[2, 6]),
                                                "cv_2" = as.numeric(df[2,7]),
                                                "Total - De 1 ano a menos de 2 anos" = as.numeric(df[2, 8]),
                                                "cv_3" = as.numeric(df[2,9]),
                                                "Total - 2 anos ou mais" = as.numeric(df[2,10]),
...
colarbh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Menos de 1 mês" = as.numeric(df[3,4]),
                                              "cv_1" = as.numeric(df[3,5]),
                                              "Total - De 1 mês a menos de 1 ano" = as.numeric(df[3, 6]),
                                              "cv_2" = as.numeric(df[3,7]),
                                              "Total - De 1 ano a menos de 2 anos" = as.numeric(df[3, 8]),
                                              "cv_3" = as.numeric(df[3,9]),
                                              "Total - 2 anos ou mais" = as.numeric(df[3,10]),
...
RIDE<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Total - Menos de 1 mês" = as.numeric(df[4,4]),
                                           "cv_1" = as.numeric(df[4,5]),
                                           "Total - De 1 mês a menos de 1 ano" = as.numeric(df[4, 6]),
                                           "cv_2" = as.numeric(df[4,7]),
                                           "Total - De 1 ano a menos de 2 anos" = as.numeric(df[4, 8]),
                                           "cv_3" = as.numeric(df[4,9]),
                                           "Total - 2 anos ou mais" = as.numeric(df[4,10]),
...
sulmg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                        ...
                                        13,26,39)], function(df) {
                                          data.frame(
                                            ...
                                            "Total - Menos de 1 mês" = as.numeric(df[5,4]),
                                            "cv_1" = as.numeric(df[5,5]),
                                            "Total - De 1 mês a menos de 1 ano" = as.numeric(df[5, 6]),
                                            "cv_2" = as.numeric(df[5,7]),
                                            "Total - De 1 ano a menos de 2 anos" = as.numeric(df[5, 8]),
                                            "cv_3" = as.numeric(df[5,9]),
                                            "Total - 2 anos ou mais" = as.numeric(df[5,10]),
...
trng<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Total - Menos de 1 mês" = as.numeric(df[6,4]),
                                           "cv_1" = as.numeric(df[6,5]),
                                           "Total - De 1 mês a menos de 1 ano" = as.numeric(df[6, 6]),
                                           "cv_2" = as.numeric(df[6,7]),
                                           "Total - De 1 ano a menos de 2 anos" = as.numeric(df[6, 8]),
                                           "cv_3" = as.numeric(df[6,9]),
                                           "Total - 2 anos ou mais" = as.numeric(df[6,10]),
...
zonamata<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                           ...
                                           13,26,39)], function(df) {
                                             data.frame(
                                               ...
                                               "Total - Menos de 1 mês" = as.numeric(df[7,4]),
                                               "cv_1" = as.numeric(df[7,5]),
                                               "Total - De 1 mês a menos de 1 ano" = as.numeric(df[7, 6]),
                                               "cv_2" = as.numeric(df[7,7]),
                                               "Total - De 1 ano a menos de 2 anos" = as.numeric(df[7, 8]),
                                               "cv_3" = as.numeric(df[7,9]),
                                               "Total - 2 anos ou mais" = as.numeric(df[7,10]),
...
nortemg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Menos de 1 mês" = as.numeric(df[8,4]),
                                              "cv_1" = as.numeric(df[8,5]),
                                              "Total - De 1 mês a menos de 1 ano" = as.numeric(df[8, 6]),
                                              "cv_2" = as.numeric(df[8,7]),
                                              "Total - De 1 ano a menos de 2 anos" = as.numeric(df[8, 8]),
                                              "cv_3" = as.numeric(df[8,9]),
                                              "Total - 2 anos ou mais" = as.numeric(df[8,10]),
...
riodoce<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Menos de 1 mês" = as.numeric(df[9,4]),
                                              "cv_1" = as.numeric(df[9,5]),
                                              "Total - De 1 mês a menos de 1 ano" = as.numeric(df[9, 6]),
                                              "cv_2" = as.numeric(df[9,7]),
                                              "Total - De 1 ano a menos de 2 anos" = as.numeric(df[9, 8]),
                                              "cv_3" = as.numeric(df[9,9]),
                                              "Total - 2 anos ou mais" = as.numeric(df[9,10]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              "Período" = as.character(df[10, 12]),
                                              "Total" = as.numeric(df[10,2]),
                                              "cv" = as.numeric(df[10,3]),
                                              "Total - Menos de 1 mês" = as.numeric(df[10,4]),
                                              "cv_1" = as.numeric(df[10,5]),
                                              "Total - De 1 mês a menos de 1 ano" = as.numeric(df[10, 6]),
                                              "cv_2" = as.numeric(df[10,7]),
                                              "Total - De 1 ano a menos de 2 anos" = as.numeric(df[10, 8]),
                                              "cv_3" = as.numeric(df[10,9]),
                                              "Total - 2 anos ou mais" = as.numeric(df[10,10]),
...
baseestratatualV6385<-list("01-Belo Horizonte"=bh,"02-Entorno metropolitano de BH"=entornobh,"03-Colar metropolitano de BH"=colarbh,
...
saveRDS(baseestratatualV6385,file = "C:/Users/italo/Desktop/trimestral/baseestratatualV6385.rds")
