...
calcula_ocup_desocup <- function(mesano){
  pnadc <- pnadc_design(read_pnadc(paste0("C:/Users/italo/Desktop/trimestral/txt/PNADC_0",mesano,".txt"), 
    ...
    update(
           ...
           one = 1,
           ocupadapr = 1 * (one == 1 & V2010 == 2),
           ocupadapd = 1 * (one == 1 & V2010 == 4),
           ocupadab = 1 * (one == 1 & V2010 == 1),
  ...
  ## Total - Branca
  estimativas_1 <- svyby(~ocupadab, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_1)[3]<-"cv_1"
  
  
  ## Total - Preta
  estimativas_2<- svyby(~ocupadapr, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  colnames(estimativas_2)[3]<-"cv_2"
  
  ## Total - Parda
  estimativas_3 <- svyby(~ocupadapd, by = ~regioes, subset(pnadc, UF =="31"), vartype="cv", na.rm = TRUE, svytotal)
  ...
  total_ocupada1<- svytotal(~ocupadab, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada2<- svytotal(~ocupadapr, subset(pnadc, UF=="31"),na.rm = TRUE)
  total_ocupada3<- svytotal(~ocupadapd, subset(pnadc,UF=="31"), na.rm = TRUE)
  ...
  total_mg<-tibble(
    ...
    ocupadab = coef(total_ocupada1),
    cv_1 = round(cv(total_ocupada1)*100,1),
    ocupadapr = coef(total_ocupada2),
    cv_2 = round(cv(total_ocupada2)*100,1),
    ocupadapd = coef(total_ocupada3),
  ...
  rm(estimativas_t, estimativas_1, estimativas_2, estimativas_3, pnadc)
  gc()
...
mg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Total - Branca" = as.numeric(df[11, 4]),
                                         "cv" = as.numeric(df[11,5]),
                                         "Total - Preta" = as.numeric(df[11, 6]),
                                         "cv" = as.numeric(df[11,7]),
                                         "Total - Parda" = as.numeric(df[11,8]),
...
bh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                     ...
                                     13,26,39)], function(df) {
                                       data.frame(
                                         ...
                                         "Total - Branca" = as.numeric(df[1, 4]),
                                         "cv" = as.numeric(df[1,5]),
                                         "Total - Preta" = as.numeric(df[1, 6]),
                                         "cv" = as.numeric(df[1,7]),
                                         "Total - Parda" = as.numeric(df[1,8]),
...
entornobh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                            ...
                                            13,26,39)], function(df) {
                                              data.frame(
                                                ...
                                                "Total - Branca" = as.numeric(df[2, 4]),
                                                "cv" = as.numeric(df[2,5]),
                                                "Total - Preta" = as.numeric(df[2, 6]),
                                                "cv" = as.numeric(df[2,7]),
                                                "Total - Parda" = as.numeric(df[2,8]),
...
colarbh<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Branca" = as.numeric(df[3, 4]),
                                              "cv" = as.numeric(df[3,5]),
                                              "Total - Preta" = as.numeric(df[3, 6]),
                                              "cv" = as.numeric(df[3,7]),
                                              "Total - Parda" = as.numeric(df[3,8]),
...
RIDE<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Total - Branca" = as.numeric(df[4, 4]),
                                           "cv" = as.numeric(df[4,5]),
                                           "Total - Preta" = as.numeric(df[4, 6]),
                                           "cv" = as.numeric(df[4,7]),
                                           "Total - Parda" = as.numeric(df[4,8]),
...
sulmg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                        ...
                                        13,26,39)], function(df) {
                                          data.frame(
                                            ...
                                            "Total - Branca" = as.numeric(df[5, 4]),
                                            "cv" = as.numeric(df[5,5]),
                                            "Total - Preta" = as.numeric(df[5, 6]),
                                            "cv" = as.numeric(df[5,7]),
                                            "Total - Parda" = as.numeric(df[5,8]),
...
trng<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                       ...
                                       13,26,39)], function(df) {
                                         data.frame(
                                           ...
                                           "Total - Branca" = as.numeric(df[6, 4]),
                                           "cv" = as.numeric(df[6,5]),
                                           "Total - Preta" = as.numeric(df[6, 6]),
                                           "cv" = as.numeric(df[6,7]),
                                           "Total - Parda" = as.numeric(df[6,8]),
...
zonamata<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                           ...
                                           13,26,39)], function(df) {
                                             data.frame(
                                               ...
                                               "Total - Branca" = as.numeric(df[7, 4]),
                                               "cv" = as.numeric(df[7,5]),
                                               "Total - Preta" = as.numeric(df[7, 6]),
                                               "cv" = as.numeric(df[7,7]),
                                               "Total - Parda" = as.numeric(df[7,8]),
...
nortemg<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Branca" = as.numeric(df[8, 4]),
                                              "cv" = as.numeric(df[8,5]),
                                              "Total - Preta" = as.numeric(df[8, 6]),
                                              "cv" = as.numeric(df[8,7]),
                                              "Total - Parda" = as.numeric(df[8,8]),
...
riodoce<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Branca" = as.numeric(df[9, 4]),
                                              "cv" = as.numeric(df[9,5]),
                                              "Total - Preta" = as.numeric(df[9, 6]),
                                              "cv" = as.numeric(df[9,7]),
                                              "Total - Parda" = as.numeric(df[9,8]),
...
central<-do.call(rbind, lapply(pnadcrds[c(1,14,27,40,
                                          ...
                                          13,26,39)], function(df) {
                                            data.frame(
                                              ...
                                              "Total - Branca" = as.numeric(df[10, 4]),
                                              "cv" = as.numeric(df[10,5]),
                                              "Total - Preta" = as.numeric(df[10, 6]),
                                              "cv" = as.numeric(df[10,7]),
                                              "Total - Parda" = as.numeric(df[10,8]),
